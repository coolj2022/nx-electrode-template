...
export function addCoreRoute(server: ElectrodeFastifyInstance) {
  const homeRenderer: PageRenderer = new PageRenderer({
    ...
    subApps: [{ name: Layout.name, ssr: false }],
    templateInserts: {
      head: {
        ...
        contextReady: [
          (context) =>
            `<link${context.user.styleNonceAttr} href="https://unpkg.com/tailwindcss@^2/dist/tailwind.min.css" rel="stylesheet">`,
          (context) =>
            `<style${context.user.styleNonceAttr}>:root{--header-height:51px;--side-nav1-width:92px;--side-nav2-width:256px}*,:after,:before{box-sizing:border-box}body,html{width:100%;height:100%;margin:0}</style>`,
        ],
      },
    },
  });
